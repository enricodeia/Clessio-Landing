...
(function () {
  ...
  function initGUI(){
    ...
    if(_isMobile&&window.lil){
      ...
      fM.add(P,'modelRotOffsetZ',-2,2,0.01).name('Rot Offset Z');

      var fI=gui.addFolder('Idle');
      fI.add(P,'idleEnabled').name('Enabled');
      fI.add(P,'idleBobSpeed',0,0.003,0.0001).name('Bob Speed');
      fI.add(P,'idleBobAmplitude',0,0.1,0.002).name('Bob Amp');
      fI.add(P,'idleSwaySpeed',0,0.001,0.00005).name('Sway Speed');
      fI.add(P,'idleSwayAmplitude',0,0.05,0.001).name('Sway Amp');

      var fL=gui.addFolder('Lights');
      fL.addColor(P,'ambientColor').name('Ambient Color').onChange(function(){if(ambientLight)ambientLight.color.set(P.ambientColor);});
      fL.add(P,'ambientIntensity',0,3,0.01).name('Ambient Intensity').onChange(function(){if(ambientLight)ambientLight.intensity=P.ambientIntensity;});
      for(var si=1;si<=3;si++){(function(n){
        var fs=fL.addFolder('Spot '+n);
        fs.add(P,'spot'+n+'Intensity',0,15,0.1).name('Intensity').onChange(function(){if(spotLights[n-1])spotLights[n-1].intensity=P['spot'+n+'Intensity'];});
        fs.add(P,'spot'+n+'X',-5,5,0.1).name('X').onChange(function(){if(spotLights[n-1])spotLights[n-1].position.x=P['spot'+n+'X'];});
        fs.add(P,'spot'+n+'Y',-5,5,0.1).name('Y').onChange(function(){if(spotLights[n-1])spotLights[n-1].position.y=P['spot'+n+'Y'];});
        fs.add(P,'spot'+n+'Z',-5,15,0.1).name('Z').onChange(function(){if(spotLights[n-1])spotLights[n-1].position.z=P['spot'+n+'Z'];});
        fs.add(P,'spot'+n+'Penumbra',0,1,0.01).name('Penumbra').onChange(function(){if(spotLights[n-1])spotLights[n-1].penumbra=P['spot'+n+'Penumbra'];});
        fs.add(P,'spot'+n+'Angle',0,Math.PI/2,0.01).name('Angle').onChange(function(){if(spotLights[n-1])spotLights[n-1].angle=P['spot'+n+'Angle'];});
        fs.addColor(P,'spot'+n+'Color').name('Color').onChange(function(){if(spotLights[n-1])spotLights[n-1].color.set(P['spot'+n+'Color']);});
      })(si);}

      var fPP=gui.addFolder('Post-Processing');
      fPP.add(P,'enableBloom').name('Bloom').onChange(function(){if(bloomPass)bloomPass.enabled=P.enableBloom;});
      fPP.add(P,'bloomStrength',0,3,0.01).name('Bloom Strength').onChange(function(){if(bloomPass)bloomPass.strength=P.bloomStrength;});
      fPP.add(P,'bloomRadius',0,1,0.01).name('Bloom Radius').onChange(function(){if(bloomPass)bloomPass.radius=P.bloomRadius;});
      fPP.add(P,'bloomThreshold',0,1,0.01).name('Bloom Threshold').onChange(function(){if(bloomPass)bloomPass.threshold=P.bloomThreshold;});
      fPP.add(P,'toneMappingExposure',0,3,0.01).name('Exposure').onChange(function(){if(renderer)renderer.toneMappingExposure=P.toneMappingExposure;});
      fPP.add(P,'enableVignette').name('Vignette').onChange(function(){if(vignettePass)vignettePass.enabled=P.enableVignette;});
      fPP.add(P,'vignetteOffset',0,2,0.01).name('Vignette Offset').onChange(function(){if(vignettePass)vignettePass.uniforms.offset.value=P.vignetteOffset;});
      fPP.add(P,'vignetteDarkness',0,2,0.01).name('Vignette Dark').onChange(function(){if(vignettePass)vignettePass.uniforms.darkness.value=P.vignetteDarkness;});
      fPP.add(P,'enableFilmGrain').name('Film Grain').onChange(function(){if(filmPass)filmPass.enabled=P.enableFilmGrain;});
      fPP.add(P,'filmGrainIntensity',0,0.5,0.005).name('Grain Intensity').onChange(function(){if(filmPass)filmPass.uniforms.intensity.value=P.filmGrainIntensity;});

      var fR=gui.addFolder('Room');
      fR.addColor(P,'wallColor').name('Wall Color').onChange(function(){buildRoom();});
      fR.add(P,'noiseAmount',0,2,0.01).name('Wall Noise').onChange(function(){buildRoom();});
      fR.add(P,'roughness',0,1,0.01).name('Roughness').onChange(function(){if(wallMat)wallMat.roughness=P.roughness;});
      fR.add(P,'metalness',0,1,0.01).name('Metalness').onChange(function(){if(wallMat)wallMat.metalness=P.metalness;});
      fR.addColor(P,'sceneBgColor').name('BG Color').onChange(function(){if(scene)scene.background=new THREE.Color(P.sceneBgColor);});

      var fLogo=gui.addFolder('Logo');
      fLogo.add(P,'logoVisible').name('Visible').onChange(function(){if(logoMesh)logoMesh.visible=P.logoVisible;});
      fLogo.add(P,'logoX',-3,3,0.01).name('X').onChange(function(){if(logoMesh)logoMesh.position.x=P.logoX;});
      fLogo.add(P,'logoY',-2,2,0.01).name('Y').onChange(function(){if(logoMesh)logoMesh.position.y=P.logoY;});
      fLogo.add(P,'logoZ',-5,5,0.01).name('Z').onChange(function(){if(logoMesh)logoMesh.position.z=P.logoZ;});
      fLogo.add(P,'logoScale',0.1,4,0.01).name('Scale').onChange(function(){if(logoMesh)logoMesh.scale.set(P.logoScale,P.logoScale,1);});
      fLogo.add(P,'logoEmission',0,100,0.5).name('Emission').onChange(function(){if(logoMaterial)logoMaterial.emissiveIntensity=P.logoEmission;});

      var fPh=gui.addFolder('Physics');
      fPh.add(P,'dragSensitivity',0.001,0.02,0.001).name('Drag Sens');
      fPh.add(P,'friction',0.5,0.99,0.005).name('Friction');
      fPh.add(P,'maxVelocity',0.01,0.5,0.01).name('Max Velocity');
      fPh.add(P,'throwMultiplier',0.5,4,0.1).name('Throw Mul');
      fPh.add(P,'easeBackDelay',0,5,0.1).name('Ease-back Delay');
      fPh.add(P,'easeBackSpeed',0.01,0.2,0.005).name('Ease-back Speed');

      // Showroom transition
      var fST=gui.addFolder('Showroom');
      fST.add(GS,'cameraZ',5,120,0.5).name('Zoom Out Z').onChange(function(){if(gridMode)gridTargetCamZ=GS.cameraZ;});
      fST.add(GS,'cameraLerp',0.005,0.5,0.005).name('Camera Ease');
      fST.add(GS,'lightMul',0,2,0.01).name('Light Mul').onChange(function(){if(gridMode)gridTargetLightMul=GS.lightMul;});
    }
  }
